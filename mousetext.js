 ...
 */function MouseTextEffect(props) {// Property controls
    ...
    React.useEffect(() => {
        if (!props.animateGlow || isOnCanvas) return; let frame, start; function animate(ts) {
            if (!start) start = ts; const t = (ts - start) / 1e3;// Pulsate between 0.95 and 1.05
            setPulse(.95 + .1 * (.5 + .5 * Math.sin(t * 2))); frame = requestAnimationFrame(animate);
        } frame = requestAnimationFrame(animate); return () => cancelAnimationFrame(frame);
    }, [props.animateGlow, isOnCanvas]);// Mouse/touch handler
    const handlePointer = React.useCallback(e => {
        if (isOnCanvas) return; setIsHovering(true); if (centerAnimRef.current) cancelAnimationFrame(centerAnimRef.current); const rect = containerRef.current.getBoundingClientRect(); let clientX, clientY; if (e.touches && e.touches.length > 0) { clientX = e.touches[0].clientX; clientY = e.touches[0].clientY; } else { clientX = e.clientX; clientY = e.clientY; } const x = clientX - rect.left; const y = clientY - rect.top; let horizontal, vertical; if (x > rect.width / 2) { horizontal = (x - rect.width / 2) / (rect.width / 2) * -1; } else { horizontal = (rect.width / 2 - x) / (rect.width / 2); } if (y > rect.height / 2) { vertical = (y - rect.height / 2) / (rect.height / 2) * -1; } else { vertical = (rect.height / 2 - y) / (rect.height / 2); }// Clamp to [-1, 1]
        horizontal = Math.max(-1, Math.min(1, horizontal)); vertical = Math.max(-1, Math.min(1, vertical));// Throttle with RAF
        if (rafRef.current) cancelAnimationFrame(rafRef.current); rafRef.current = requestAnimationFrame(() => { setDirection({ horizontal, vertical }); });
    }, [isOnCanvas]);// Mouse leave handler: animate back to center
    const handlePointerLeave = React.useCallback(() => {
        setIsHovering(false); if (centerAnimRef.current) cancelAnimationFrame(centerAnimRef.current); function animateBack() {
            setDirection(prev => {
                const speed = .18; const h = prev.horizontal + (1 - prev.horizontal) * speed; const v = prev.vertical + (1 - prev.vertical) * speed; if (Math.abs(h - 1) < .01 && Math.abs(v - 1) < .01) {// Snap exactly to center and stop animating
                    if (centerAnimRef.current) cancelAnimationFrame(centerAnimRef.current); return { horizontal: 1, vertical: 1 };
                } else { centerAnimRef.current = requestAnimationFrame(animateBack); return { horizontal: h, vertical: v }; }
            });
    ...
    function parseColor(input) {
        if (!input) return { r: 255, g: 255, b: 255 }; if (input.startsWith("rgb")) {// rgb or rgba
            const match = input.match(/rgba?\(([^)]+)\)/); if (match) { const parts = match[1].split(",").map(Number); return { r: parts[0] || 255, g: parts[1] || 255, b: parts[2] || 255 }; }
        } else if (input.startsWith("#")) {
            let c = input.replace("#", ""); if (c.length === 3) { c = c[0] + c[0] + c[1] + c[1] + c[2] + c[2]; } if (c.length === 6) {// Pad with zeros if needed
                c = c.padStart(6, "0"); const r = parseInt(c.slice(0, 2), 16); const g = parseInt(c.slice(2, 4), 16); const b = parseInt(c.slice(4, 6), 16); return { r, g, b };
            }// 8-digit hex (with alpha)
            if (c.length === 8) {
                c = c.padStart(8, "0"); const r = parseInt(c.slice(0, 2), 16); const g = parseInt(c.slice(2, 4), 16); const b = parseInt(c.slice(4, 6), 16);// ignore alpha for now
                return { r, g, b };
            }
        }// fallback: white
    ...
    const glowOffset = .7 * (typeof font.fontSize === "number" ? font.fontSize : parseInt(font.fontSize) || 160) / 160// scale with font size
        ; const x1 = (direction.horizontal || 0) * glowOffset; const y1 = (direction.vertical || 0) * glowOffset; const x2 = (direction.horizontal || 0) * glowOffset * 2; const y2 = (direction.vertical || 0) * glowOffset * 2; const x3 = (direction.horizontal || 0) * glowOffset * .5; const y3 = (direction.vertical || 0) * glowOffset * .5; const mainTextStyle = {
    ...
    React.useEffect(() => {
        if (typeof window === "undefined") return; if (document.getElementById("mouse-text-shadow-style")) return; const style = document.createElement("style"); style.id = "mouse-text-shadow-style"; style.innerHTML = `
